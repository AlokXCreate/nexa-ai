...
import { MODEL_CATALOG } from './marketplace.controller';

export async function profileHardware(req: AuthRequest, res: Response) {
  try {
    ...
    const profile = await prisma.deviceProfile.upsert({
      where: {
        id: (await prisma.deviceProfile.findFirst({ where: { userId: req.user.id } }))?.id || 'new-profile'
      },
      update: {
        os,
        cpuCores,
    ...
    const recommendations = MODEL_CATALOG.map(model => {
      let score = 100;
      let status: 'OPTIMAL' | 'COMPATIBLE' | 'LAGGY' | 'UNSUPPORTED' = 'OPTIMAL';
...
export async function getDeviceProfile(req: AuthRequest, res: Response) {
  try {
    if (!req.user) return res.status(401).json({ error: 'Unauthorized' });
