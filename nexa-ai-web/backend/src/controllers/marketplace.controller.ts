...
export async function getModelCatalog(req: AuthRequest, res: Response) {
  try {
    return res.json(MODEL_CATALOG);
  } catch (err: any) {
    return res.status(500).json({ error: err.message });
  }
}

export async function getInstalledModels(req: AuthRequest, res: Response) {
  try {
...
export async function simulateModelDownload(req: AuthRequest, res: Response) {
  try {
    if (!req.user) return res.status(401).json({ error: 'Unauthorized' });
    const { modelId } = req.body;

    const exists = MODEL_CATALOG.find(m => m.id === modelId);
    if (!exists) return res.status(404).json({ error: 'Model not found in catalog' });
