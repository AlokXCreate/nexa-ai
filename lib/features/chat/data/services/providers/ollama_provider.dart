...
class OllamaProvider implements UniversalAiProvider {
  final Dio _dio = Dio();
  final CloudConfigRepository _configRepo;

  OllamaProvider(this._configRepo);

  @override
  String get id => 'ollama';

  @override
  String get name => 'Ollama Local/Remote Server';
  ...
  Stream<ProviderStreamChunk> generateStream({
    ...
    runZonedGuarded(() async {
      ...
      if (config == null || !config.isEnabled) {
        throw Exception('Ollama provider is disabled or missing.');
      }

      final modelId = modelOverride ?? config.defaultModelId;
      final endpoint = '${config.baseUrl}/chat';
      ...
      if (response == null || response.data == null) {
        throw Exception('Ollama failed to respond.');
      }
...
final ollamaAiProvider = Provider<OllamaProvider>((ref) {
  final repo = ref.watch(cloudConfigRepositoryProvider);
  return OllamaProvider(repo);
});
