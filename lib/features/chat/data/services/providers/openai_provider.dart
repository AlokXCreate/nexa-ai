...
class OpenAiProvider implements UniversalAiProvider {
  final Dio _dio = Dio();
  final CloudConfigRepository _configRepo;

  OpenAiProvider(this._configRepo);

  @override
  String get id => 'openai';

  @override
  String get name => 'OpenAI Compatible API';
  ...
  Stream<ProviderStreamChunk> generateStream({
    ...
    runZonedGuarded(() async {
      ...
      if (config == null || !config.isEnabled) {
        throw Exception('OpenAI provider is disabled or missing.');
      }
      ...
      if (response == null || response.data == null) {
        throw Exception('OpenAI failed to respond.');
      }
  ...
  Future<ProviderResponse> generate({
    ...
    stream.listen(
      ...
      onDone: () {
        // Construct final ToolCalls from deltas
